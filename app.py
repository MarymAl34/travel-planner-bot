...
أنت مساعد تخطيط سفر محترف، أنشئ خطة مفصّلة باللغة العربية فقط.
...
لا تذكر أنك نموذج ذكاء اصطناعي، وقدّم الخطة بشكل عملي وواضح.
"""
            else:
                user_prompt = f"""
...
Return a detailed travel plan in **English only**, using clear Markdown with:

1. **Trip overview**
2. **Day-by-day itinerary** (morning / afternoon / evening for each of the {days} days)
3. **Hotel recommendations** (luxury / mid-range / budget)
4. **Estimated budget breakdown**
5. **Restaurant & café suggestions**
6. **Top attractions & activities**
7. **Travel tips & reminders**
8. **Packing checklist**

Do not mention that you are an AI model. Make reasonable assumptions where needed.
"""
            ...
            try:
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are a helpful, structured and concise travel planning assistant.",
                        },
                ...
                st.error(t("حدث خطأ أثناء الاتصال بنموذج الذكاء الاصطناعي.", "An error occurred while contacting the AI model."))
                st.exception(e)
else:
    st.markdown(
        t(
            "اضغط على زر **«خطِّط رحلتي»** بعد تعبئة البيانات لرؤية الخطة هنا.",
            "Fill in the trip details, then click **“Plan my trip”** to see your plan here.",
        )
    )

st.markdown("</div>", unsafe_allow_html=True)  # نهاية كارت النتيجة
